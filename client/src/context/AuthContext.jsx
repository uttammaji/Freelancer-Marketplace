...
export function AuthProvider({ children }) {
    ...
    const refreshUser = async () => {
      try {
        ...
        if (response.success && response.user) {
          ...
          localStorage.setItem('skillhire_user', JSON.stringify(response.user));
        }
      } catch (error) {
  ...
  const uploadAvatar = useCallback(async (file) => {
    ...
    try {
      ...
      const uploadResult = await uploadService.uploadImage(compressedFile, 'avatars', 'avatar');
      if (!uploadResult.success) return { success: false, error: 'Upload failed' };
      ...
      if (updateResult.success && updateResult.user) {
        const updatedUser = { ...currentUser, avatar: updateResult.user.avatar, avatarPublicId: updateResult.user.avatarPublicId };
  ...
  const removeAvatar = useCallback(async () => {
    try {
      ...
      if (updateResult.success) {
        const updatedUser = { ...currentUser, avatar: '', avatarPublicId: null };
        setCurrentUser(updatedUser);
        localStorage.setItem('skillhire_user', JSON.stringify(updatedUser));
      }
      return { success: true };
  ...
  const verifyPhoneOTP = useCallback(async (phone, otp) => {
    try {
      ...
      if (response.success && response.user) {
        const updatedUser = { ...currentUser, phone: response.user.phone, isPhoneVerified: response.user.isPhoneVerified };
        setCurrentUser(updatedUser);
        localStorage.setItem('skillhire_user', JSON.stringify(updatedUser));
      }
      return { success: true, data: response };
    } catch (error) {
      return { success: false, error: error.response?.data?.message || 'Failed to verify phone' };
    }
  }, [currentUser]);
  ...
  const changePhone = useCallback(async (newPhone) => {
    try {
      ...
      return { success: false, error: error.response?.data?.message || 'Failed to initiate phone change' };
    }
  }, []);

  const verifyOldPhoneOTP = useCallback(async (newPhone, oldPhoneOTP) => {
    try {
      const response = await authService.verifyOldPhoneOTP(newPhone, oldPhoneOTP);
      return { success: true, data: response };
    } catch (error) {
      return { success: false, error: error.response?.data?.message || 'Failed to verify old phone OTP' };
    }
  }, []);

  const verifyNewPhoneOTP = useCallback(async (newPhone, newPhoneOTP) => {
    try {
      const response = await authService.verifyNewPhoneOTP(newPhone, newPhoneOTP);
      if (response.success && response.user) {
        const updatedUser = { ...currentUser, phone: response.user.phone, isPhoneVerified: response.user.isPhoneVerified };
        setCurrentUser(updatedUser);
        localStorage.setItem('skillhire_user', JSON.stringify(updatedUser));
      ...
      return { success: false, error: error.response?.data?.message || 'Failed to verify new phone OTP' };
    }
  }, [currentUser]);
  ...
  const changeEmail = useCallback(async (newEmail) => {
    try {
      ...
      return { success: false, error: error.response?.data?.message || 'Failed to initiate email change' };
    }
  }, []);

  const verifyEmailChange = useCallback(async (newEmail, oldEmailOTP, newEmailOTP) => {
    try {
      const response = await authService.verifyEmailChange(newEmail, oldEmailOTP, newEmailOTP);
      if (response.success && response.user) {
      ...
      return { success: false, error: error.response?.data?.message || 'Failed to verify email change' };
    }
  }, [currentUser]);

  // ============ RESEND OTP HELPERS ============

  const resendRegistrationOTP = useCallback(async (email) => authService.resendRegistrationOTP(email), []);
  const resendLoginOTP = useCallback(async (email) => authService.resendLoginOTP(email), []);
  const resendResetOTP = useCallback(async (email) => authService.resendResetOTP(email), []);
  ...
  const value = useMemo(() => ({
    ...
    isLoading,
    register, verifyRegistration, login, verifyLogin, forgotPassword, resetPassword,
    logout, updateProfile, changePassword, changeUsername,
    resendRegistrationOTP, resendLoginOTP, resendResetOTP,
    profile, isProfileLoading, fetchProfile, saveProfile,
    uploadAvatar, removeAvatar, isAvatarUploading,
    sendPhoneOTP, verifyPhoneOTP, changePhone, verifyOldPhoneOTP, verifyNewPhoneOTP,
    changeEmail, verifyEmailChange,
  }), [
    currentUser, token, isLoading, profile, isProfileLoading, isAvatarUploading,
    logout, updateProfile, changePassword, changeUsername,
    fetchProfile, saveProfile, uploadAvatar, removeAvatar,
    resendRegistrationOTP, resendLoginOTP, resendResetOTP,
    sendPhoneOTP, verifyPhoneOTP, changePhone, verifyOldPhoneOTP, verifyNewPhoneOTP,
    changeEmail, verifyEmailChange,
  ]);
